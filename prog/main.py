...
# 出力フィアルの保存名
OUTPUT_FILE_NAME = "sample_01.wav"
...
class C_spectrumAnalyzer():
    def __init__(self):
        ...
        self.wave_list["N"] = self.my_dtmf.createSineWave(
            DTMF_AMPLITUDE, 0, 0, 8000.0, self.chunk / (self.fs*1.0))
...
#         while 1:
        # 適当に5秒間実行して終了
        for n in xrange(0, self.fs/self.chunk * 5):
            try:
                # dataは文字列型
        ...
        print("DTMF音楽作成中...")
        wave_data = ""
        for scale in sound_data:
            print(scale)
            wave_data += self.wave_list[scale]

        self.my_dtmf.save(wave_data, 8000, 16, OUTPUT_FILE_NAME)
        print("DTMFに変換したものを %s として保存しました。" % (OUTPUT_FILE_NAME))
...
if __name__ == "__main__":
    ...
    w_player = PLAY_WAVE()
    w_player.play(OUTPUT_FILE_NAME)
